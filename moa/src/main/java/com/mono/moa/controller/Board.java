...
BoardDao bDao;
	...
	//게시글 리스트 보기 요청 처리함수
	@RequestMapping("/allboardlist.moa")
	public ModelAndView nboardList(ModelAndView mv, PageUtil page) {
		int total = bDao.getTotal();
		
		page.setPage(page.getNowPage(), total, 10, 10);
		
		List list = bDao.nboardList(page);
		for(Object o : list) {
